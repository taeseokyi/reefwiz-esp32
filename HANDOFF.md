# 작업 인계 (2026-08-19 — 보드 확정 · 화면/SD 제거 · 장기 저장소 · 원본 대조 감사 · 정비 UI 재구성)
...
```bash
python3 tools/test_measure_sim.py      # ★122체크 ALL PASS (약 5분)
...
1. **★BIND 주소 입력** — 실제 HC-06 주소를 넣어야 전환이 동작한다(비어 있으면 의도적으로
   거부 — 빈 주소로 아무 데나 붙는 것 방지). ★2026-08-19부터 **정비페이지에서 넣는다**
   (BT 연결 → 접속 정보 설정, MAC 붙여넣기 가능). 소스 수정·재배포 불요이고,
   `config.BIND_ADDR_*` 는 폴백으로만 남는다.
