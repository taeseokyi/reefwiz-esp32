# reefwiz-esp32 앱 — WiFi/NTP → 웹서버(스레드) → 스케줄 루프(메인 스레드).
# ★2026-09-24: 부팅 진입점 main.py 는 이제 webota 범용 런처다(WiFi 최소 접속 → 원격 배포 서버
#   :8266 → `import app; app.main()`). 이 파일은 import 해도 돌지 않는다 — 런처가 main() 을
#   부른다. 앱이 죽어도 원격 배포 서버는 살아 있다(상세: README '원격 배포').
...
def main():
    ...
    #   리셋은 회차를 깨고, 다음 회차 직전 리셋은 그 회차를 통째로 건너뛰게 한다. 파일 API
    #   (조회·수정·삭제)는 가드를 거치지 않는다. 급하면 클라이언트 --force.
    try:
        import webota
        webota.set_guard(lambda: ops.guard_measure(120, "배포·리셋")[:2])
    except ImportError:
        pass                                   # 런처 없이 돌 때(옛 배치·PC) — 가드만 없다
