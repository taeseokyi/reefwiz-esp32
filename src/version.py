...
# ★C-1 = 1세대 **제어기**(Controller). 측정기(Meter M-1)·도징기(Doser D-1)와 같은 계열
#   표기다 — 이 기기 자체는 KH 를 재지 않고 두 장비를 물고 돌며 시키는 쪽이라,
#   측정 대상(KH)을 모델명에 넣으면 하는 일과 어긋난다(2026-08-30 사용자 지적).
#   제어 구성이 바뀌면 C-2 가 된다.
# (ESP32-S3 + HC-05 로 측정기·도징기를 물고 도는 구성). 하드웨어 구성이 바뀌면 KH-2 가 된다.
