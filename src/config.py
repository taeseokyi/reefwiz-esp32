...
# ★★실장 경고(2026-08-28 실측): **GPIO21 로 EN 을 LOW 로 내려도 이 보드에서는 HC-05 전원이
#   꺼지지 않았다**(연결 유지·STATE=1 지속, ESP32 전체 전원을 내려야만 끊김). 즉 지금 배선에서는
#   `_power_cycle`/`_pulse_reset`(전원 경로 전환·하드리셋)이 **무효**다. 고속 경로(KEY)와 데이터
#   모드 자동연결은 정상 동작하므로 운영에는 지장이 없으나, 라디오가 좀비가 됐을 때의 최후 복구
#   수단이 없는 상태다 — EN 배선을 점검할 것(EN 이 실제로 물려 있는지/납땜 불량).
BAUD = 9600
