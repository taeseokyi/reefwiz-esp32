...
#   ★2026-08-29 실측으로 배선이 단순해졌다 — **납땜 불필요, 헤더 6핀만 쓴다**:
#   - BT_KEY_PIN(GPIO21) → 헤더의 `EN` 핀. 이 보드에서 EN 은 전원이 아니라 **KEY(PIN34)** 다.
#     HIGH=AT 모드 / LOW=데이터 모드로 완전히 제어된다(실측). 종전처럼 코어 PIN34 에 선을
#     납땜할 필요가 없다.
#   - BT_STATE_PIN(GPIO38) → 헤더의 `STATE`. 연결=HIGH / 미연결=LOW (실측 검증).
#   - BT_POWER_PIN → **없다**. EN 이 KEY 이므로 전원 차단 수단이 없다(전원 재투입이 필요하면
#     VCC 를 MOSFET/스위치로 게이팅해야 한다 — 현재는 수동 토글 스위치).
#     ※다른 ZS-040 리비전에는 EN 이 LDO(LP2985) ON/OFF 에 물린 것도 있다(EN—1K—노드—220K—VCC).
#       그 보드라면 EN 이 전원 핀이고 KEY 는 PIN34 납땜이 필요하다 — 실물로 확인할 것.
BAUD = 9600
...
# ★★2026-08-29 실측 확정 — 헤더 `EN` = **KEY(PIN34)** 다. 전원 차단 핀이 아니다:
#   ①EN 을 LOW 로 내려도 모듈 전원은 안 꺼진다 → 전원 경로 무효(BT_POWER_PIN=None).
#   ②EN 으로 KEY 를 **완전히 제어할 수 있다**: HIGH→AT 모드(OK 응답), LOW→데이터 모드 복귀
#     (도저 응답 정상). ⇒ **PIN34 납땜이 불필요**하다. 헤더 6핀만으로 끝난다.
#   ★2026-08-28 에 "EN 은 LOW 를 못 내린다"고 적었던 것은 오진이었다 — 그때는 HC-05 가 이미 AT
#     모드에 갇혀 있었고 전원을 끊을 수단이 없어 빠져나오지 못한 것이다(전원 스위치 설치 후
#     콜드 부팅하니 EN 만으로 완전 동작 확인).
#   ★KEY 선을 두 개(GPIO14+EN) 물리면 서로 싸워 AT 진입이 깨진다 — 반드시 하나만 연결한다.
BT_KEY_PIN = 21                 # HC-05 KEY = 헤더의 `EN` 핀(실측 확정). None 이면 대상 전환 불가
...
# ── 도저 펌웨어 계열 장치 여러 대 (★2026-08-21 사용자 요구) ──
# ★정정(2026-08-30): 여기서 '추가 도징기'로 부르던 실물은 **에어 분배기**다
#   (ReefWiz Air A-1 / RWA1 — 도저 펌웨어를 응용해 4구 솔레노이드 밸브를 순차 제어한다).
#   명령·응답 규약이 도저와 같아 등록·시계 동기 경로가 그대로 맞는다. 상세는 devices.py 헤더.
...
# ★2026-08-29 실측: 재바인드(RMAAD+BIND) 만으로는 **60초를 기다려도 새 대상에 붙지 않는다**.
#   반면 **HC-05 전원을 껐다 켜면 약 1초 만에** BIND 대상으로 자동 연결된다. 즉 이 펌웨어에서
#   대상 전환의 마지막 단계는 **전원 콜드 부팅이 필수**다(AT+LINK 는 FAIL, AT+RESET 도 부족).
#   ⇒ 무인 자동 전환을 하려면 VCC 를 끊는 하드웨어(MOSFET/로드스위치)가 필요하다.
#     현재는 수동 토글 스위치라, 전환 시 운영자가 한 번 눌러야 한다.
#   대기값은 전원 재투입 직후의 연결 확인용이다 — **실측 5초면 넉넉하다**(스위치 조작 시점부터
#   BIND 대상에 붙기까지). 8초로 두어 여유만 조금 준다(종전 12초는 재바인드-후-대기 용도로
#   잡혀 있어 의미가 어긋나 있었다).
BT_CONNECT_SECS = 8.0           # AT+RESET 후 자동 연결 확인 대기(실측 0.4~5.0초)
BT_SWITCH_TRIES = 3             # 전환 재시도 횟수
BT_AT_TIMEOUT = 2.0             # AT 명령 응답 대기
# 장치 목록에서 접속 암호를 비워 뒀을 때 쓰는 기본 PIN(HC-06 공장값).
# ★비운 칸을 '지금 값 그대로'로 두면 안 된다(2026-08-29 시뮬 검증): 암호를 넣은 장비에 붙은
#   뒤 안 넣은 장비로 전환하면 마스터에 남은 PIN 이 그대로 쓰여 **조용히 못 붙는다**.
#   빈 칸 = 이 기본값이라고 못 박아 대상마다 PIN 이 항상 확정되게 한다.
BT_DEFAULT_PSWD = "1234"
