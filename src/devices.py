...
#
# ★★★정정(2026-08-30): 지금 '도저 2번' 자리에 등록돼 있는 실물은 **도징기가 아니라
#   에어 분배기**다(ReefWiz Air A-1 / RWA1). 도저 펌웨어를 응용해 **4구 솔레노이드 밸브를
#   순차 제어**해 에어를 나눠 보내는 장치라, **명령과 구조가 도징기와 거의 동일**하다
#   (응답 서명까지 같다 — 그래서 지금은 도저와 갈라낼 방법이 없다).
#   → 이 파일의 '도징기 N대' 는 정확히는 **'도저 펌웨어 계열 장치 N대'** 로 읽어야 한다.
#   ※동작은 이미 옳다: 추가 장치는 아래 규칙대로 '시계 동기 전용'이고, 에어 분배기 역시
#     자체 타이머로 도는 장치라 필요한 것이 시계 동기뿐이다(lrt·자동조정은 애초에 차단).
#   ※`ver` 규약(README '장비 펌웨어 ver 규약')이 펌웨어에 들어오면 **이름**으로 종류가
#     갈린다 — 그때 KINDS 에 에어 분배기를 별도 종류로 분리할 수 있다.
#
