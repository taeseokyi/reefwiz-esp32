...
class Link:
    ...
    def leave_at_mode(self, tries=3):
        ...
        for i in range(tries):
            if self.enter_at(prefer_connected=False) is None:
                ...
                # ★★연결을 **아예 기다리지 않는다**(사용자 지적 2026-08-30: "연결을 할 필요없어,
                #   검색인 건데"). 여기서 알아야 하는 건 '붙었나'가 아니라 '데이터 모드인가'다.
                #   붙는 일은 다음 작업(측정·도저·대상 전환)이 필요할 때 하고, 그때 신원까지
                #   검증한다. 여기서 기다리면 검색이 끝났는데도 화면이 몇 초 더 잡혀 있다.
                self._reset_into_data(wait_secs=0)
    ...
    def inquire(self, secs=25.0, max_secs=None, on_found=None, should_stop=None,
        ...
        ★조회 **전**에는 리셋하지 않지만, 조회가 끝나면 **데이터 모드로 확실히 되돌린다**
          (2026-08-30 사용자 관찰: "검색 중 AT 모드일 때도 있다 — 원래 빠르게 깜빡여야 한다").
          검색은 KEY 를 몇 분간 올린 채 도는데, 그 사이 모듈이 한 번이라도 재부팅하면 Way-2
          AT 모드(느린 깜빡임)로 올라오고 **KEY 를 내리는 것만으로는 거기서 못 나온다**.
          그 상태로 끝나면 이후 측정·도징 명령이 통째로 안 나간다. 그래서 빠져나갈 때
          `AT+RESET` 을 보내고 **부팅되는 동안 KEY 를 내려** 데이터 모드로 부팅시킨다
          (`_rebind_key` 의 마지막 단계와 같은 규약). 그 부팅에서 BIND 자동연결이 다시 걸린다.
        ...
        ★★이름 조회는 **KEY(PIN34)가 HIGH 인 동안에만** 응답한다 — 문서로 확인된 이 모듈의
          성질이다(Martyn Currey, ZS-040 실측: "RNAME only works for me when pin 34 is HIGH.
          When pin 34 is LOW or not connected I get nothing, no reply and no error message").
          예전 세션이 "RNAME 20초 무응답"으로 기록한 원인이 이것이다(KEY 를 내린 채 물었다).
          그래서 이름 조회는 **반드시 이 AT 세션 안에서**(KEY 를 올려 둔 채) 해야 한다 —
          조회가 끝난 뒤 따로 부르는 구조로 바꾸면 조용히 무응답이 된다.
        ★조회 응답 자체에는 **이름이 없다**(2026-08-30 두 모드 실측 + 데이터시트 확인 —
          이름을 함께 주지 않을까 하는 것이 자연스러운 기대라 확인해 뒀다):
              INQM=1(RSSI) → `+INQ:98DA:60:561E1,1F00,FFB6`  (주소, 클래스, RSSI)
              INQM=0(표준) → `+INQ:98DA:60:561E1,1F00,7FFF`  (RSSI 자리가 7FFF)
          둘 다 세 칸뿐이다(데이터시트도 `+INQ:<주소>,<device class>,<RSSI>` 세 칸으로 기술).
          그래서 이름은 반드시 RNAME 왕복이 필요하고, 주소가 먼저 뜨고 이름이 뒤늦게 채워지는
          화면 동작은 그 때문이다. '이름만 먼저 받고 주소는 나중에'는 성립하지 않는다.
