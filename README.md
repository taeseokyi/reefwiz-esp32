...
**부품 명세** (전부 관통형이라 브레드보드에 그대로 꽂힌다):

| 기호 | 부품 | 패키지 | 규격 | 수량 | 비고 |
|---|---|---|---|---|---|
| Q1 | **IRF9540N** (P채널 MOSFET) | TO-220 | Vds −100V, Id −23A, Rds(on) 0.117Ω @Vgs −4.5V | 1 | 디바이스마트 상품번호 `11781`. `IRF4905`·`IRF9540` 도 가능 |
| Q2 | **2N7000** (N채널 MOSFET) | TO-92 | Vds 60V, Id 200mA, 논리레벨 | 1 | 상품번호 `12089`. `2N7002`(SMD)·`2N3904`(NPN) 대체 가능 |
| R1 | 저항 **10 kΩ** | 1/4W | — | 1 | Q1 게이트 풀업(전원 OFF 유지) |
| R2 | 저항 **1 kΩ** | 1/4W | — | 1 | GPIO → Q2 게이트 직렬(보호) |
| R3 | 저항 **10 kΩ** | 1/4W | — | 1 | Q2 게이트 풀다운(부팅 중 OFF 확정) |
| C1 | 전해 커패시터 **100 µF** | 16V 이상 | 유극성 | 1 | HC-05 VCC↔GND. 전원 재투입 시 돌입전류 완화 |
| C2 | 세라믹 커패시터 **0.1 µF** | 50V | 무극성 | 1 | HC-05 VCC↔GND. 고주파 노이즈 제거 |
...
| 10 | C1 (100µF) | **+** → HC-05 VCC(=Q1 Drain), **−** → GND ★극성 주의 |
| 11 | C2 (0.1µF) | HC-05 VCC ↔ GND (무극성) |
