...
| **앰버(주황) 상시** | 경고(측정은 계속됨) | WiFi 끊김/AP 폴백 모드, 또는 **BT 대상 미검증**(원격 장비 꺼짐/전환 실패) |
| **빨강 깜빡** | 치명(측정이 멈췄거나 위협받음) | **에러 래치**(프로브 보호로 측정 정지) · **BT 링크 동결**(신원 불일치) · **시각 미동기**(정시 측정 게이트 닫힘) |
...
- **경고 vs 치명의 기준**: 이 장비의 주 목적은 측정이다. 측정이 계속되는 저하(WiFi·BT 미검증)는
  앰버, 측정이 실제로 멈춘/막힌 상태(에러 래치·링크 동결·시각 없음)는 빨강 깜빡으로 구분한다.
