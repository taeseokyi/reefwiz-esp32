...
| **주변 장치 검색** | `AT+INQ` 로 주변 BT 장치의 **주소**를 훑는다(작업 `bt_scan`, 정비페이지 → BT 연결 → 장치 목록 → '🔍 주변 장치 검색'). 새 도징기·에어 분배기를 등록하려면 그 HC-06 주소를 알아야 하는데, 실장된 기기로 알아낼 방법이 이것뿐이다(종전에는 PC 에 HC-05 를 따로 물려야 했다). ★★**조회 전에 `AT+RESET` 을 하면 안 된다**(2026-08-30 실측 — 첫 구현이 그래서 실패했다): 소프트 리셋으로 AT 모드에 부팅하면 `AT+STATE?` 가 `INITIALIZED` 에서 멈추고 `AT+INIT` 이 늘 `ERROR:(17)`, `AT+INQ` 는 `ERROR:(1F)` 로 거부된다(ROLE/CMODE/CLASS/IAC/RMAAD 조합을 다 바꿔도 동일). KEY↑ 로 들어가 **리셋 없이** 바로 조회하면 그대로 된다 — 벤치 도구 `hc05_cmode1.scan()` 이 쓰던 절차다. **연결도 끊기지 않는다**('연결 중이라 조회가 안 된다'는 틀린 가정이었다). 약 40초 걸리며 측정 중·모터 구동 중에는 거부한다. 조회에 필요한 `CMODE=1` 은 빠져나갈 때 반드시 `CMODE=0` 으로 되돌린다(아무 장비에나 붙는 모드로 남기지 않는다). 이미 등록된 주소에는 장치 이름을 붙여 보여 준다 | 웹 |
