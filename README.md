...
| **표시 이름** | `ReefWiz Controller C-1 (A1B2C3)` — 괄호 안이 **개체**. `ver` 한 줄의 `#` 와 헷갈리면 안 된다(거기는 **빌드 커밋**) |
| **판(버전)** | `MAJOR.MINOR.PATCH+커밋` (예: `1.0.0+3f2a1c9`). 단일 진실은 `src/version.py` |

**버전 조회** — 화면 없는 장비라 다음 경로가 전부 같은 값을 보여 준다:

```bash
curl http://reefwiz.local/api/ver         # 한 줄만 → ReefWiz Controller C-1 v1.0.0 #7833c9c
...
- 부팅 로그 첫 줄 `[boot] ReefWiz Controller C-1 v1.0.0 #7833c9c | 개체 4D6F04 · … 배포`
...
| **주변 장치 검색** | `AT+INQ` 로 주변 BT 장치의 **주소**를 훑는다(작업 `bt_scan`, 정비페이지 → BT 연결 → 장치 목록 → '🔍 주변 장치 검색'). 새 도징기·에어 분배기를 등록하려면 그 HC-06 주소를 알아야 하는데, 실장된 기기로 알아낼 방법이 이것뿐이다(종전에는 PC 에 HC-05 를 따로 물려야 했다). ★★**조회 전에 `AT+RESET` 을 하면 안 된다**(2026-08-30 실측 — 첫 구현이 그래서 실패했다): 소프트 리셋으로 AT 모드에 부팅하면 `AT+STATE?` 가 `INITIALIZED` 에서 멈추고 `AT+INIT` 이 늘 `ERROR:(17)`, `AT+INQ` 는 `ERROR:(1F)` 로 거부된다(ROLE/CMODE/CLASS/IAC/RMAAD 조합을 다 바꿔도 동일). KEY↑ 로 들어가 **리셋 없이** 바로 조회하면 그대로 된다 — 벤치 도구 `hc05_cmode1.scan()` 이 쓰던 절차다. **연결도 끊기지 않는다**('연결 중이라 조회가 안 된다'는 틀린 가정이었다). 약 40초 걸리며 측정 중·모터 구동 중에는 거부한다. ★**지금 HC-05 가 붙어 있는 장비는 결과에 나오지 않는다** — 연결 중인 상대는 조회에 응답하지 않는다(2026-08-30 확인: 같은 기기가 한 패스에서는 잡히고 다음 패스에서는 빠졌는데, 그 사이 바인드 대상에 자동 연결된 것이 원인이었다). 결과 끝에 그 사실을 붙여 준다 — 안 적으면 '장비가 사라졌다'로 읽혀 멀쩡한 배선을 의심하게 된다. ★**`CMODE=1` 은 쓰지 않는다**: 벤치 도구가 조회 전에 그걸 넣길래 따라 했다가, 연속 검색이 도는 몇 분 동안 모듈이 **검색 중 발견한 장비에 실제로 붙어 버렸다**(2026-08-30). `CMODE=0`(바인드 주소 전용)으로도 조회가 그대로 된다 — 앞서 CMODE=0 에서 났던 `ERROR:(1F)` 는 CMODE 탓이 아니라 `AT+RESET` 탓이었다. 조회 내내 `CMODE=0` 을 유지하고, 빠져나갈 때 한 번 더 못 박는다. 이미 등록된 주소에는 장치 이름을 붙여 보여 준다 | 웹 |
...
### 장비 펌웨어 `ver` 규약 (합의 2026-08-30 — 측정기·도징기 실장 확인 완료)

측정기·도징기·에어 분배기 펌웨어가 **기기명 + 판 + 빌드 커밋**을 한 줄로 내보낸다. 형식:

```
ReefWiz Meter M-1 v1.0.0 #55DAFC
└─ 이름 ────────┘ └ 판 ┘ └ 커밋 ┘
```

★`#` 는 **빌드 커밋**이다(2026-08-30 확정). 장비 펌웨어가 한 저장소에서 함께 빌드되므로
세 장비가 같은 값을 낸다 — 정상이다. 처음 이 자리를 '개체'로 적었던 것은 오독이었다.
**개체(어느 대인가)는 이 줄에 없다** — 아래 미해결 항목 참조.

| | 이름 | 코드 | 비고 |
|---|---|---|---|
| 제어기(이 ESP32) | ReefWiz Controller C-1 | `RWC1` | `src/version.py` — 이미 구현. **`ver` 명령은 없다**(묻는 쪽이라 시리얼 명령을 받지 않는다) — 같은 한 줄을 `GET /api/ver`(한 줄 텍스트)·`/api/version` 의 `ver` 필드로 낸다 |
| 측정기 | ReefWiz Meter M-1 | `RWM1` | `reefwiz_ph_meter_final.ino` |
| 도징기 | ReefWiz Doser D-1 | `RWD1` | 실물은 올포리프 도저(`PRIMARY_DOSER_NAME`) |
| 에어 분배기 | ReefWiz Air A-1 | `RWA1` | ★**도저 펌웨어를 응용**해 4구 솔레노이드 밸브를 순차 제어한다. 명령과 구조가 도징기와 **거의 동일**하다 — 응답 서명(`ls` → "왼쪽 동작")까지 같아서 이름이 유일한 구분 수단이다. `ver` 은 `ReefWiz Air A-1 v1.0.0 #<커밋>` |
...
- **제어기도 같은 한 줄을 낸다** — `curl http://reefwiz.local/api/ver`, `/api/version` 의
  `ver` 필드, 정비페이지 '장비 정보' 카드의 `ver 한 줄`, 부팅 로그 첫 줄. 넷(제어기·측정기·
  도징기·에어 분배기)이 같은 모양이라 나란히 표시할 때 파서가 하나면 된다. 제어기의 `#` 도
  **빌드 커밋**이다(배포 스탬프의 해시) — 개체는 `/api/version` 의 `serial` 에 따로 있다.
- **이름이 종류를 가른다.** `ReefWiz Doser D-1` 과 `ReefWiz Air A-1` 은 `ls` 응답이 글자
  그대로 같아 서명으로는 갈라낼 수 없었는데, `ver` 이름으로는 갈린다. 그동안 lrt·자동조정을
  기본 도저 1대로 묶어 둔 이유가 바로 그 구분 불가였다(`config.py`·`devices.py` 헤더).
- **대수(어느 대인가)는 아직 못 가른다.** `#` 가 빌드 커밋이라 같은 종류 2대는 같은 값을
  낸다. 지금 그 구분을 하는 것은 BIND 주소뿐이다(`link.bound_addr`). 도저를 2대 이상 두려면
  `ver` 에 개체 토큰을 하나 더 붙이거나(예: 칩 ID·HC-06 주소 뒤 3바이트), 지금처럼 BIND
  주소에 맡긴다.

#### 실장 확인 (2026-08-30, 실기 192.168.0.47 / COM4)

측정기·도징기 펌웨어에 `ver` 이 들어갔고, 제어기가 붙는 즉시 한 줄을 읽어 두는 것까지
실기에서 확인했다(정비페이지 '장비 정보' → 연결 장비의 판):

```
제어기   ReefWiz Controller C-1 v1.0.0 #4D6F04   ← GET /api/ver
측정기   ReefWiz Meter M-1 v1.0.0 #55DAFC        ← 명령 콘솔 `ver` 원문·자동 캡처 일치
도징기   ReefWiz Doser D-1 v1.0.0 #55DAFC        ← 자동 캡처
```

측정기와 도징기가 같은 `#55DAFC` 를 내는데, 이는 **빌드 커밋**이라 정상이다(장비 펌웨어가
한 저장소에서 함께 빌드된다). 에어 분배기도 같은 커밋이면 `ReefWiz Air A-1 v1.0.0 #55DAFC`
가 된다.

★**미해결 — 같은 종류 2대는 `ver` 로 갈리지 않는다.** `#` 가 커밋이므로 도저를 2대 두면 둘
다 같은 한 줄을 낸다. 지금 그 구분은 BIND 주소가 맡는다(`link.bound_addr` — 서명으로 종류,
주소로 개체). 개체를 `ver` 로도 갈라야 한다면 장비 펌웨어에 토큰을 하나 더 붙여야 한다
(칩 고유 ID · HC-06 주소 뒤 3바이트 · EEPROM 일련번호 중 하나).

★**에어 분배기는 아직 등록돼 있지 않다** — 실기 장치 목록은 측정기·도저 2대뿐이고
(`ids: ['meas','doser']`), 저장소 어디에도 그 BT 주소가 없다. 주소를 장치 목록에 넣으면
`ver` 수집·시계 동기가 다른 장비와 똑같이 돈다.

펌웨어가 나온 뒤 이쪽에서 할 일: ①`devices.KINDS` 에 `ver` 프로브 추가 ②`link` 신원 검증에
이름·개체 대조 ③**에어 분배기를 별도 종류로 분리**(지금은 도저로 등록해야 한다 — UI 라벨도
"도징기 추가"뿐이다) ④정비페이지 **장비 정보** 카드에 연결된 장비들의 판 표시
