...
| **장비 상태 표시** | 측정 중 / 조치 작업 / BT 동결 / 에러 래치 / 액체 위치 불명 / 대기(Idle)를 페이지 맨 위에 색과 함께 한 줄로. 위 표 참조 | 웹 |
...
- 우선순위: **치명(빨강) > 경고(앰버) > 정상(소등)**. 메인(측정) 루프가 매 틱 상태를 계산해
...
### ★PIN34(KEY) 는 어디인가 — 코어 모듈 오른쪽 맨 위 핀

![HC-05 PIN34 위치](docs/hc05-pin34.svg)

**방향 기준**(이게 없으면 "오른쪽"이 뜻을 잃는다): 부품면(금속 캔·안테나 패턴이 보이는 면)을
위로 두고, **6핀 헤더를 아래**로 향하게 놓는다. 그러면 안테나 패턴이 위쪽 끝에 온다.
그 상태에서 **코어 모듈 오른쪽 열의 맨 위 패드가 PIN34(PIO11)** = KEY 다.

납땜은 둘 중 어디에 해도 같다(같은 네트):

1. **코어 모듈 오른쪽 맨 위 패드(PIN34)** — 위 도면의 빨간 표시.
2. **온보드 버튼의 VCC 아닌 쪽 패드** — 패드가 커서 납땜이 쉽다. 버튼이 `VCC → PIN34` 를
   잇는 구조이므로 이쪽도 전기적으로 PIN34 다.

**배선 전 확인**: 멀티미터 도통 모드로 **버튼을 누른 상태**에서 VCC 와 도통되는 쪽이 PIN34 다.
버튼이 VCC-PIN34 를 잇는 구조라 이 검사 하나로 확정된다 — 캐리어 리비전마다 버튼 위치가
조금씩 달라도(대개 헤더 반대편 모서리) 이 방법은 그대로 통한다.
...
- **★KEY 는 헤더에 없다** — ZS-040 온보드 버튼의 두 패드 중 **VCC 가 아닌 쪽(PIN34/PIO11)** 에
  ...
  ★PIN34 자체는 **코어 모듈 오른쪽 맨 위 패드**다(헤더를 아래로 놓았을 때) —
  위 'PIN34 는 어디인가' 절의 도면 참조.
...
| GPIO14 | **PIN34/PIO11**(코어 모듈 오른쪽 맨 위 패드 또는 버튼 패드) | ★납땜. 10k 풀다운 권장 |
...
| 부품 | 규격 | 수량 | 비고 |
|---|---|---|---|
| P채널 MOSFET | **AO3401A** (SOT-23, Vds -30V, Id -4A, Vgs(th) -0.9V) | 1 | 논리레벨. `DMG3415U`·`AO3407` 도 가능 |
| N채널 MOSFET | **2N7002** (SOT-23) 또는 NPN `2N3904` | 1 | 게이트 구동용 반전 단 |
| 저항 | **10 kΩ** | 2 | R1(P-FET 게이트 풀업), R3(N-FET 게이트 풀다운) |
| 저항 | **1 kΩ** | 1 | R2(GPIO→N-FET 게이트 직렬) |
| 커패시터 | 100 µF 전해 + 0.1 µF 세라믹 | 각 1 | HC-05 VCC 옆에 둔다(재투입 시 돌입전류·전압강하 완화) |

**핀 배열** — AO3401A·2N7002 **둘 다 SOT-23 이고 핀 번호가 같다**(데이터시트 확인):
**①Gate ②Source ③Drain**. 부품 글자면을 위로 두고 **다리 2개인 쪽을 아래**로 놓으면:
...
```
        ...
        │    2N7002     │
        └───────────────┘
         ①Gate    ②Source          ← 다리 2개(아래), 왼쪽이 ①
```

※ 핀 번호는 같아도 **역할이 반대**다: P-FET 은 ②Source 가 **+5V** 쪽, N-FET 은 ②Source 가
   **GND** 쪽이다. 반드시 아래 연결표대로 꽂는다(둘은 채널이 반대라 서로 대체 불가).
   2N7002 는 연속 200mA 까지라 HC-05(≈40mA)에 충분하다.
   TO-92 패키지(2N3904 등)로 대체하면 평평한 면을 앞으로 볼 때 **좌:E 중:B 우:C** 이고,
   E→GND, B→R2(GPIO), C→P-FET 게이트로 대응한다.
...
|---|---|
| **P-FET ② Source** | **+5V** |
| **P-FET ③ Drain** | **HC-05 VCC** (+ C1 100µF, C2 0.1µF 를 여기서 GND 로) |
| **P-FET ① Gate** | R1(10k) → +5V  ·  그리고 N-FET ③ Drain |
| **N-FET ③ Drain** | P-FET ① Gate |
| **N-FET ② Source** | **GND** |
| **N-FET ① Gate** | R2(1k) → **ESP32 GPIO14**  ·  그리고 R3(10k) → GND |
| C1 100µF (+/−) | + → HC-05 VCC, − → GND (극성 주의) |
| C2 0.1µF | HC-05 VCC ↔ GND (무극성) |

**ESP32-S3 쪽 핀 배열 (최종 — HC-05 관련 전부)**:
...
| **GPIO14** | **MOSFET 게이트**(위 표 N-FET ① Gate) | HIGH=전원 ON |
| 5V | MOSFET 경유 → HC-05 VCC | 직결하지 않는다 |
| GND | HC-05 GND · MOSFET GND | **공통 GND — 절대 끊지 않는다** |

**회로** — 하이사이드 스위치(5V 라인을 끊는다. **GND 는 절대 끊지 않는다** — 신호선으로 역급전된다):
...
```
      +5V ──┬──────────────┬───────────────┐
           ...
           R1 10k          │ S             │
            │         ┌────┴────┐          │
            ├─────────┤G  P-FET │  AO3401A │   (게이트가 LOW 면 도통)
            │         └────┬────┘          │
            │              │ D             │
            │              ├──────────────────→ HC-05 VCC (+ C1 100µF, C2 0.1µF → GND)
            │              │
            │             ===  (부하: HC-05 ≈ 40mA, 피크 100mA 미만)
            │
            │   D ┌─────────┐
            └─────┤ N-FET   │ 2N7002
                  │    G  S │
...
**동작**: `GPIO14=HIGH` → N-FET 도통 → P-FET 게이트가 LOW → **HC-05 전원 ON**.
`GPIO14=LOW` → N-FET 차단 → R1 이 게이트를 5V 로 올림 → **HC-05 전원 OFF**.
★부팅 중 GPIO 가 부동이어도 R3 가 게이트를 내려 **기본 OFF** 다. 부팅 직후 코드가 HIGH 로 올려
켜므로, 전원 인가와 동시에 한 번 콜드 부팅되는 셈이라 오히려 편하다.
(반대 극성을 원하면 R1 을 GND 쪽으로 옮기는 대신 **N-FET 단을 빼고** P-FET 게이트를 GPIO 로 직접
몰면 안 된다 — 3.3V 로는 5V 소스의 P-FET 을 확실히 끄지 못한다. 반전 단이 필요한 이유다.)

**적용 후 설정**:

```python
# config.py
BT_POWER_PIN = 14               # MOSFET 게이트(위 회로의 GPIO14)
BT_POWER_ACTIVE_HIGH = True     # HIGH=ON
BT_POWER_OFF_SECS = 0.5         # 방전 대기(0.4~1.0초면 충분)
```

이 두 줄이면 `link._power_cycle()` 이 살아나 **대상 전환·좀비 복구가 완전 자동**이 된다.
...
| G. 스케줄·장치 관리 | 회차 검증(간격 2h 미만·**자정을 넘는 원형 간격**·중복·13개·조정 회차 목록 밖 전부 거부), 하루 1회는 저장되되 경고, `rows_cap` 이 회차 수에 따라 84→336, **긴 회차가 종료 슬롯을 소비해 연속 측정이 안 나는지**, `/api/schedule` 계약, **도징기 2대에서 전환이 동결되지 않는지**(같은 종류 서명은 남의 것이 아니다) + 도저2 에서 도징량 조작 거부, 장치별 `set time` 도달, 장치를 지우면 '붙어 있다'는 상태도 버리는지 |
