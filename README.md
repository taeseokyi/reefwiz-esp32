...
**★GPIO32 는 반드시 VCC 스위치여야 한다(EN 이 아니다).** HC-05 의 KEY 핀은 AT 모드 진입에
써야 하므로, 하드 리셋은 전원을 실제로 끊는 방식뿐이다 — 부품과 회로도는 아래
"HC-05 전원 스위치 회로" 절 참조.
...
## HC-05 전원 스위치 회로 (GPIO32)

전원을 실제로 끊었다 넣는 것이 AT 모드 진입의 유일한 방법이라 스위치가 필요하다.
**반드시 VCC(하이사이드)를 끊어야 한다.** GND 를 끊는 로우사이드 방식은 여기서 못 쓴다 —
VCC 가 붙어 있는 채로 GND 만 뜨면, ESP32 의 TX 핀에서 HC-05 입력 보호 다이오드를 타고
전류가 흘러 모듈이 반쯤 살아 있는 상태가 된다. 그러면 전원 재투입으로 모드가 바뀌지 않는다.

### 권장 — 2석 하이사이드 스위치 (5V 급전, GPIO HIGH = ON)

| 부품 | 예시 품번 | 비고 |
|---|---|---|
| P-채널 MOSFET | **AO3401A** (SOT-23) / IRLML6402 / DMG3415U | 로직레벨. HC-05 는 ~40mA 라 여유 충분 |
| N-채널 소신호 MOSFET | **2N7002** (SOT-23) / **2N7000** (TO-92, 브레드보드용) | 레벨 시프트 겸 반전 |
| 저항 | 10kΩ ×1, 1kΩ ×2, 100kΩ ×1 | |
| 커패시터 | 10µF(전해/탄탈) + 100nF(세라믹) | HC-05 VCC 바로 옆에 |

```
   +5V ──┬──────────────── S │P-MOSFET (AO3401A)
         │                    │
        R1 10k                D ──┬── HC-05 VCC
         │                        │
         ├──── G ────────┐       C1 10µF ─┬─ C2 100nF
...
동작: GPIO32 **HIGH** → N-FET ON → P-FET 게이트가 GND 로 → Vgs=-5V → P-FET ON → 전원 인가.
GPIO32 **LOW** → N-FET OFF → R1 이 게이트를 5V 로 올림 → Vgs=0 → 차단.

- **R4(100k 풀다운)가 핵심이다.** ESP32 부팅 중 GPIO32 는 잠깐 뜬 상태(하이임피던스)가 되는데,
  이게 없으면 그 사이 모듈 전원이 제멋대로 들어갔다 나갔다 한다.
- **KEY 라인(GPIO33)에도 10k 풀다운을 달 것.** 부팅 글리치로 KEY 가 뜬 채 전원이 들어가면
  모듈이 AT 모드로 부팅해 버려서 장비에 안 붙는다.
- C1 은 SD 카드와 HC-05 가 같은 3.3V 레귤레이터를 쓸 때 특히 중요하다(돌입 전류 완충).

`config.py`: `BT_POWER_ACTIVE_HIGH = True` (기본값)

### 간단 대안 — 1석 (3.3V 급전, GPIO LOW = ON)

HC-05 모듈을 3.3V 로 직접 먹일 수 있는 보드라면 N-FET 없이 P-MOSFET 게이트를 GPIO 로 직접
구동해도 된다(소스 3.3V, 게이트 0V → Vgs=-3.3V → ON). 부품은 P-MOSFET 1개 + 10k 풀업 1개.

```
  +3.3V ──┬───── S │P-MOSFET ── D ── HC-05 VCC
          R1 10k     │
          └───────── G ── GPIO32
```

`config.py`: `BT_POWER_ACTIVE_HIGH = False`

**★5V 급전에는 이 방식을 쓰면 안 된다.** GPIO 가 3.3V 까지밖에 못 올라가는데 소스가 5V 라
Vgs=-1.7V 가 남아 P-MOSFET 이 완전히 꺼지지 않는다. 반쯤 켜진 채로 남아 모드 전환이 실패한다.

### ★먼저 확인할 것 — 보드의 6번 핀이 KEY 인가 EN 인가

HC-05 브레이크아웃은 종류가 갈린다:

- **JY-MCU 계열**: 6번 핀이 `KEY` = 칩의 **PIO11**. 이게 우리가 원하는 핀이다 → GPIO33 에 직결.
- **ZS-040 계열**(가장 흔함, 온보드 버튼 있음): 헤더의 `EN` 은 **레귤레이터 enable** 이라
  AT 모드와 무관하다. PIO11 은 **온보드 버튼**에 연결돼 있으므로, 버튼 패드(3.3V 쪽이 아닌
  PIO11 쪽)에 선을 납땜해 GPIO33 에 물려야 한다.

구분법: 6번 핀을 3.3V 에 물린 채 전원을 넣어 38400 으로 `AT` 를 보냈을 때 `OK` 가 오면 KEY 다.
`OK` 가 안 오고 그냥 평소처럼 동작하면 그 핀은 EN 이므로 버튼 패드를 찾아야 한다.

HC-05 의 KEY/PIO11 은 3.3V 로직이라 ESP32 와 직결해도 된다(레벨 시프터 불요).
