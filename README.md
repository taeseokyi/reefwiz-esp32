...
BT 연결         현재 대상 · 마지막 응답 확인 · RF 이벤트 · STATE · BIND 주소
                [측정기로 전환][도저로 전환][연결 점검][HC-05 리셋][동결 해제]
                ▸ 접속 정보 설정   (BIND 주소 — MAC 붙여넣기 가능)
...
현재 대상으로만 나가며(`ops.current_target()`), 도징은 **대상이 도저일 때만** 열린다
(서버도 같은 규칙으로 거부 — `_require_doser`). 빠른 명령 버튼도 대상에 맞는 세트만 보인다.
...
| **BT 접속 정보 설정** | 상대 HC-06 의 BIND 주소를 웹에서 넣는다(2026-08-19). MAC 을 그대로 붙여 넣어도 된다 — `98:DA:60:0F:C5:7A` / `98DA600FC57A` / `98da,60,0fc57a` 가 모두 같은 값으로 정규화된다. `/data/bt.json` 에 저장되고 **`config.py` 값보다 우선**한다(비우면 config 폴백). 백업 번들에도 포함 | 웹 |
...
tools/
  ...
  test_archive.py    아카이브·백업/복원 단위 검증(38체크, 하드웨어 불요)
firmware/
...
| `/api/bt` | GET/POST | BT 접속 정보(BIND 주소) — POST `{"meas": "98:DA:...", "doser": "..."}`. MAC 형식 자동 변환, 빈 값=지움(config 폴백). `config.BIND_ADDR_*` 보다 우선 |
...
```bash
python3 tools/test_measure_sim.py     # 측정 시퀀스·조치·관리 122체크
python3 tools/test_archive.py         # 장기 저장소·백업/복원 31체크(수초)
```

**결과(2026-08-19): 측정·조치·관리 122체크 + 아카이브 38체크 ALL PASS**
