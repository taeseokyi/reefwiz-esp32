...
| **주변 장치 검색** | `AT+INQ` 로 주변 BT 장치의 **주소**를 훑는다(작업 `bt_scan`, 정비페이지 → BT 연결 → 장치 목록 → '🔍 주변 장치 검색'). 새 도징기·에어 분배기를 등록하려면 그 HC-06 주소를 알아야 하는데, 실장된 기기로 알아낼 방법이 이것뿐이다(종전에는 PC 에 HC-05 를 따로 물려야 했다). ★★**운영자가 HC-05 전원 스위치를 껐다 켜야 한다** — 이 모듈(펌웨어 3.0-20170601)은 "KEY HIGH 인 채 **전원 인가**"로 들어간 AT 모드에서만 조회를 받는다. `AT+RESET` 으로 들어간 AT 모드는 반쪽이라 `AT+STATE?` 가 `INITIALIZED` 에서 멈추고 `AT+INIT` 이 늘 `ERROR:(17)`, `AT+INQ` 는 `ERROR:(1F)` 다(2026-08-30 실측 — ROLE/CMODE/CLASS/IAC/RMAAD 조합을 다 바꿔도 동일). 작업이 그 순간을 최대 90초 기다리다가 켜지면 자동으로 검색한다(판정 신호는 `AT+INIT` 이 OK 를 내는 것). VCC 를 MOSFET 로드 스위치로 게이팅하면 이 단계가 자동화된다. 측정 중·모터 구동 중에는 거부하고, 끝나면 직전 대상으로 다시 붙는다. 이미 등록된 주소에는 장치 이름을 붙여 보여 준다 | 웹 |
