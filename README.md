...
측정기·도징기·에어 분배기 펌웨어가 **기기명 + 판 + 개체**를 한 줄로 내보낸다. 형식:
...
| 에어 분배기 | ReefWiz Air A-1 | `RWA1` | ★**도저 펌웨어를 응용**해 4구 솔레노이드 밸브를 순차 제어한다. 명령과 구조가 도징기와 **거의 동일**하다 — 응답 서명(`ls` → "왼쪽 동작")까지 같아서, 이름이 붙기 전에는 도저와 갈라낼 방법이 없다 |
...
- **제어기도 같은 한 줄을 낸다** — `curl http://reefwiz.local/api/ver`, `/api/version` 의
  ...
  도징기·에어 분배기)이 같은 모양이라 나중에 나란히 표시할 때 파서가 하나면 된다.
- **이름이 종류를, `#개체` 가 대수를 가른다.** 지금은 둘 다 불가능하다 — 등록된 2번 장치는
  사실 도징기가 아니라 **에어 분배기**인데(2026-08-30 정정), 도저 펌웨어 파생이라 응답이
  글자 그대로 같아 `ls` 서명으로는 갈라낼 수 없다. 그래서 lrt·자동조정을 기본 도저 1대로
  묶어 뒀다(`config.py`·`devices.py` 헤더). `ver` 이 들어오면
  `ReefWiz Doser D-1` 과 `ReefWiz Air A-1` 이 **이름으로** 갈리고, 같은 종류가 여러 대일 때는
  `#개체` 가 갈라 준다 — 그때 이 제약을 풀 수 있다.
...
★**미해결 — 개체(`#`)가 장비마다 같다.** 측정기와 도징기가 **둘 다 `#55DAFC`** 를 낸다
(제어기만 MAC 뒤 3바이트라 다르다). 개체 식별자는 "같은 종류가 여러 대일 때 어느 대인가"를
가르는 값인데, 펌웨어에 상수로 박혀 있으면 그 일을 못 한다 — 도저 2대든 도저+에어 분배기든
`#` 로는 갈리지 않는다. 장비 펌웨어에서 **개체마다 다른 값**으로 만들어야 한다:
칩 고유 ID(제어기가 쓰는 방식) · HC-06 모듈의 BT 주소 뒤 3바이트 · EEPROM 에 굽는 일련번호
중 하나. 그 전까지 개체 기반 신원 검증은 열 수 없다(이름으로 종류만 갈린다).
