...
| **BT 대상 전환** | HC-05 1개를 측정기/도저에 다시 바인드하고 **신원을 검증**. 불일치면 동결(오장비 명령 방지), 해제는 운영자 확인 후에만. **측정 중·모터 구동 중에는 잠긴다** | 웹 |
...
`tools/test_measure_sim.py` 의 시나리오 E·F 가 이 넷을 전부 검증한다(오접속 동결, 동결 중 송신
차단, 모터 중 전환 거부, BIND 주소 미설정 시 거부, 측정 중 전환 거부·force 무력화).
...
- **★KEY 는 헤더에 없다** — ZS-040 온보드 버튼의 두 패드 중 **VCC 가 아닌 쪽(PIN34/PIO11)** 에
  선을 납땜해 GPIO14 로 뺀다. 헤더의 `EN` 을 KEY 로 착각하면 AT 모드에 못 들어간다.
- **★10kΩ 풀다운은 GPIO14 쪽에 단다**(GPIO14–버튼패드 선과 GND 사이). 부팅 중 GPIO 가 부동인
  순간에 KEY 가 뜨면 모듈이 AT 모드로 부팅해 장비에 붙지 않는다.
- **EN 에는 풀다운을 달지 않는다** — 보드에 220K 풀업이 있어 부동 상태가 곧 ON 이고,
  LOW 로 내리는 순간이 곧 전원 차단이다(MOSFET 불요).
...
| F. 관리 계층 | `/api/dkh` 가 **tank_kh** 를 돌려주는지(위치 인덱싱 회귀 고정)·음수/0.0 표식 통과, 로그가 상한에서 **쓰는 도중에도** 회전하는지, 도저 **수동 우선**(오버라이드 적용 회차는 자동 조정 생략), **측정 중 BT 전환 거부**(force 로도 불가·측정 경로만 통과)와 `link.status()` 보고 내용 |
